...
class Order(object):
    ...
    def func1(self, event):
        buy_market_order("QQQ.NQ", 110)
        sell_market_order("SPY.AM", 90)

    def exit1(self,event):
        sell_market_order("QQQ.NQ", 110)
        buy_market_order("SPY.AM", 90)
...
f2 = plt.figure(2,figsize=(2,4))
f2.canvas.set_window_title('ORDERS')
callback = Order()
ax1 = plt.axes([0.2, 0.8, 0.5, 0.15])
ax2 = plt.axes([0.2, 0.6, 0.5, 0.15])
ax3 = plt.axes([0.2, 0.4, 0.5, 0.15])
ax4 = plt.axes([0.2, 0.2, 0.5, 0.15])
order1 = Button(ax1, 'Long Q Short SPY')
order1.on_clicked(callback.func1)
exit1 = Button(ax2, 'exit')
exit1.on_clicked(callback.exit1)
order2 = Button(ax3, 'Long SPY Short Q')
order2.on_clicked(callback.func2)
exit2 = Button(ax4, 'exit')
exit2.on_clicked(callback.exit2)
